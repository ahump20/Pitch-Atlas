...
import { compareUrl, parseSelection } from '../components/compare/selection'
...
describe('island learning adapter', () => {
  it('exports the canonical pitch objects without copying the registry', () => {
    ...
    expect(adapter.pitchBySlug).toBe(pitchBySlug)
    expect(adapter.pitchBySlug('four-seam')).toBe(pitchBySlug('four-seam'))
  })

  it('preserves canonical comparison normalization', () => {
    expect(adapter.normalizeSelection).toBe(normalizeSelection)
    expect(adapter.parseSelection).toBe(parseSelection)
    expect(adapter.compareUrl).toBe(compareUrl)
  ...
  it('preserves query parsing and stable compare URLs', () => {
    const parsed = adapter.parseSelection('?a=four-seam&b=slider&view=movement&hand=left&orientation=side')
    expect(parsed).toEqual({ a: 'four-seam', b: 'slider', view: 'movement', hand: 'left', orientation: 'side' })
    expect(adapter.compareUrl(parsed)).toBe('/compare?a=four-seam&b=slider&view=movement&hand=left&orientation=side')
    expect(adapter.parseSelection('?a=invalid&b=slider&view=invalid')).toEqual({
      a: null, b: 'slider', view: 'grips', hand: 'right', orientation: 'top',
    })
  })

  it('uses the canonical seam and projection implementations', () => {
    expect(adapter.seamPoint).toBe(seamPoint)
    expect(adapter.projectSeam).toBe(projectSeam)
