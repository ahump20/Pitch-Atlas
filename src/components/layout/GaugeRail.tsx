...
export function GaugeRail({ gauges, caption }: { gauges: Gauge[]; caption?: string }) {
  return (
    <div className="flex flex-col">
      {gauges.map((g, i) => (
        <div key={g.key} className={i > 0 ? 'border-t border-machined/70 py-5' : 'pb-5'}>
          <div className="mono-label mb-2.5">{g.label}</div>
          <SourcedValue claim={g.claim} accent={g.accent} valueClassName="text-xl md:text-2xl" />
        </div>
      ))}
      {caption ? (
        <p className="mt-5 max-w-[34ch] border-t border-machined/70 pt-5 text-[0.8125rem] leading-snug text-dim/85">
          {caption}
        </p>
      ) : null}
    </div>
