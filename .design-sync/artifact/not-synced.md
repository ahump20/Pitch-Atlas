...
This system was built from the site's code (ahump20/Pitch-Atlas, main at 4098cbd) by running the site's own build and bundling it whole; every preview and the cover were rendered and checked. What the token table cannot carry:
...
- `.field-cream`, `.rfx-plate`, `.rfx-card` and `.scene-coal` re-tone tokens inside their own elements. They are component scopes, not page themes; the table lists the top-level values, and each usage note says what changes on cream.
