...
export function RefractorSource({ source, className = '' }: { source: Source; className?: string }) {
  return (
    <a
      ...
      className={`inline-flex min-w-0 max-w-full items-baseline gap-1 pointer-coarse:-my-[15px] pointer-coarse:py-[15px] font-mono text-[10px] uppercase tracking-[0.06em] text-ink-3 underline decoration-ink-3/40 decoration-1 underline-offset-2 transition-colors hover:text-bone hover:decoration-cyan ${className}`}
