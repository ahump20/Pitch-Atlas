...
/*
  ...
  is no windmill and no riseball shape — but it is not nothing: arc, spin to deaden
...
export const SLOWPITCH_NOTES: SlowpitchNote[] = [
  {
    ...
    claim: claim(
      ...
      'official-data',
      { note: 'A floor-and-ceiling rule on the height of the lob; the exact limits live in the sanctioning body’s rulebook.' },
    ),
  },
  {
    label: 'Why the arc limit is not one limit',
    claim: claim(
      ...
      'reputable-analysis',
      { note: 'Named rather than smoothed: different leagues (USSSA vs. USA Softball and others) publish different arc limits, so any single arc band is only true for its own rulebook.' },
    ),
  },
  {
