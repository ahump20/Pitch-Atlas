...
/* The archive opens with a stable reading plane and its original chase card.
   Native scroll moves the light and card at separate depths; the media and
   specimen view-transition identity stay with the existing card. */
export function HeroCase({ featured }: { featured: PitchAtlasEntry }) {
  ...
  return (
    <section
      ...
      className="v2-stage v2-tooth archive-case relative overflow-hidden"
      style={{ '--c3': accent.c3 } as React.CSSProperties}
    >
      <PresentationBackdrop
        asset={PRESENTATION_MEDIA.homeThreadAtmosphere}
        eager
        className="archive-case-atmosphere opacity-[0.22] mix-blend-screen md:opacity-[0.3]"
      />
      <div className="archive-case-light" aria-hidden="true" />
      ...
      <div className="v2-hero-grid relative z-[1] mx-auto grid max-w-[1320px] grid-cols-1 items-center gap-7 px-5 pb-16 pt-6 md:min-h-[calc(100dvh-4.5rem)] md:grid-cols-12 md:gap-6 md:px-8 md:pb-24 md:pt-12">
        ...
        <div className="v2-hero-read order-1 md:col-span-6 md:self-center">
          {/* the index line yields on severely short viewports (landscape phones):
              the deploy gate requires the Pitch Index action fully inside a
              568x320 first viewport, and the mark's 31px is exactly what pushed
        ...
        <div className="order-2 flex justify-center md:col-span-6 md:justify-end">
          <div className="v2-refract archive-case-card relative mx-auto w-full max-w-[290px] sm:max-w-[360px] md:mx-0 md:max-w-[460px]">
      ...
      <div className="archive-case-footer" aria-hidden="true"><span>The living collection</span><span>Grip / History / Craft</span></div>
    </section>
  )
}
