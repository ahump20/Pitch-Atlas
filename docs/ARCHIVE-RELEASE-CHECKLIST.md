...
| --- | --- | --- |
| Current unsigned learning archive | Exact53985e7 attempt stalled in `actool`/`ibtoold` while CoreSimulator loaded runtime-profile metadata; build log and helper CPU stopped progressing. Termination signals targeted only owned build processes, preserving diagnostics and partial output. The last snapshot still showed xcodebuild and ibtoold exiting, so complete reaping is not claimed. | Resolve the asset-tool/runtime metadata read obstruction, then make one new exact-source archive. The existing1e2b archive is not the current learning build. |
